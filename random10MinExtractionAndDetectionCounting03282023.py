...
def MinSecToMilliseconds(min,sec):
    return ((min*60)+sec)*1000

def extractRandom10MinFromWav(in_file,out_file,max_start_pos=-1):
    # Define constants
...
for inFile in inFiles:
    if '/2161/' in inFile:
        ...
        max_start_points.append(MinSecToMilliseconds(0,0))
    elif '/3207/' in inFile:
        inFiles_new.append(inFile)
        max_start_points.append(MinSecToMilliseconds(5, 49))
    elif '/1457/' in inFile:
        inFiles_new.append(inFile)
        max_start_points.append(MinSecToMilliseconds(4, 12))
    elif '/2991/' in inFile:
        inFiles_new.append(inFile)
        max_start_points.append(MinSecToMilliseconds(6, 55))
    elif '/3091/' in inFile:
        inFiles_new.append(inFile)
        max_start_points.append(MinSecToMilliseconds(5, 45))
    elif '/3560/' in inFile:
        inFiles_new.append(inFile)
        max_start_points.append(MinSecToMilliseconds(0, 0))
inFiles = inFiles_new
...
for i in range(len(inFiles)):
    ...
    outFile = outFolder + sdan + '_updated_by_kyunghun_03282023.wav'
    try:
    ...
    for j in range(10):
        ...
        start_pos = extractRandom10MinFromWav(inFile, outFile,max_start_points[i])
        numOfDetect = calculateNumOfDetect(outFile)
        if numOfDetect > 10: break
...
df.to_csv(outFolder + '/output_summary_03282023.csv', index=False)
