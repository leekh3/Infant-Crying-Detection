...
def extractRandom10MinFromWav(in_file,out_file,max_start_pos=-1):
    ...
    start_pos = random.randint(max_start_pos, max_start_pos)
...
for inFile in inFiles:
    ...
    if '/4045/' in inFile:
        ...
        max_start_points.append(MinSecToMilliseconds(2,27,20))
    elif '/2816/' in inFile:
        inFiles_new.append(inFile)
        max_start_points.append(MinSecToMilliseconds(10,44,6))
inFiles = inFiles_new
...
for i in range(len(inFiles)):
    ...
    outFile = outFolder + sdan + '_updated_by_kyunghun_04032023.wav'
    try:
...
df.to_csv(outFolder + '/output_summary_04032023.csv', index=False)
